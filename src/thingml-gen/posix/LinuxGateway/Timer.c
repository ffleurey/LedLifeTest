...
void Timer_enqueue_timeout(uint8_t id, uint16_t listener_id)
{
    ...
    enqueue_buf[1] = (1 >> 0) & 0xFF;
    union { uint8_t id; uint8_t bytebuffer[1] } timer_timeout_id_u;
    timer_timeout_id_u.id = id;
    memcpy(&enqueue_buf[2], timer_timeout_id_u.bytebuffer, 1);
    externalMessageEnqueue(enqueue_buf, 3, listener_id);
